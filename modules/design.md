...
Do not edit this file for project-specific design context.

Use design context in this order:

1. the existing product/codebase
2. layr.config.md, if present
3. the user's task
4. accessible neutral defaults, only when no design system exists

Preserve the existing design language unless the task explicitly asks for a redesign.
If brand tokens are missing, infer them from the codebase before creating new ones.
